          ...
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            <div className="flex items-center gap-8">
              <NavLink to="/" className="flex items-center gap-3 group">
                <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-brand-600 via-indigo-500 to-purple-400 p-0.5 shadow-glow-brand group-hover:scale-105 transition-transform duration-300">
                  <div className="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center">
                    <Sparkles className="w-5 h-5 text-brand-400 group-hover:text-white transition-colors" />
                  </div>
                </div>
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] font-semibold text-brand-400 tracking-wider uppercase -mt-1">
                    AI Financial Suite
                  </span>
                </div>
              </NavLink>
                ...
                {navLinks.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span>{item.name}</span>
                      {item.highlight && (
                        <span className="ml-1 px-1.5 py-0.5 text-[10px] font-bold uppercase rounded-md bg-gradient-to-r from-brand-500 to-indigo-500 text-white tracking-wide shadow-sm animate-pulse-subtle">
                          AI
                        </span>
                      )}
                    </NavLink>
            ...
            <div className="hidden md:flex items-center gap-3">
              {onOpenAddModal && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold text-white bg-gradient-to-r from-brand-600 to-indigo-600 hover:from-brand-500 hover:to-indigo-500 rounded-xl shadow-glow-brand transition-all duration-200"
                >
              ...
              >
                <div className="w-8 h-8 rounded-xl bg-slate-800 group-hover:bg-brand-600/30 border border-slate-700/60 group-hover:border-brand-500/40 flex items-center justify-center text-slate-300 group-hover:text-brand-300 transition-colors">
                  <UserIcon className="w-4 h-4" />
                </div>
                <div className="flex flex-col text-left">
                  <span className="text-xs font-semibold text-white truncate max-w-[110px] group-hover:text-brand-300 transition-colors">
                    {user?.name || 'User'}
                  </span>
            ...
            <div className="flex md:hidden items-center gap-2">
              {onOpenAddModal && (
                <button
                  type="button"
                  onClick={onOpenAddModal}
                  className="p-2 text-white bg-brand-600 rounded-xl shadow-glow-brand"
                >
                  <PlusCircle className="w-4 h-4" />
                </button>
              )}
            ...
            {navLinks.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.highlight && (
                    <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-brand-600 text-white uppercase">
                      AI
                    </span>
                  )}
                </NavLink>
            ...
            <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between px-2">
              ...
              >
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-brand-400">Settings & Currency ({currency.symbol})</span>
                </div>
              </button>
              <button
                onClick={handleLogout}
